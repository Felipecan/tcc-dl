...
def split_audio(path_to_audio, time_step):
    '''
        Description:
            The split_audio() function splits a determinate audio into several parts, according to the stipulated time inverval.
        ...
        Parameters:                
            ...
            step:
                Step to the audio will be divided, givin in milliseconds. E.g: 1000 equals 1000 ms, which is 1 seconds.

        Return:
            A audios list containing in each position a part from original audio.
            IN CASE THE AUDIO INEXISTENT: it'll return a empty list.        
    '''   
...
def wav2spectrogram(audio_file, path_to_save):
    ...
    frequencies, times, spectrogram = signal.spectrogram(array(audio_file.get_array_of_samples()), audio_file.frame_rate)
    plt.tick_params(top=False, bottom=False, left=False, right=False, labelleft=False, labelbottom=False)
    plt.box(False)  
    plt.tight_layout(pad=0.0, w_pad=0.0, h_pad=0.0)
    plt.pcolormesh(times, frequencies, log(spectrogram)) # 20.*np.log10(np.abs(spectrogram)/10e-6) decibel    
